  ...
  <div>
    <div class="form-background p-3">
      <b-form @submit.prevent="onSubmitUpload">
        ...
        <b-btn
          data-test-id="capsule-button-startUpdate-Active"
          type="submit"
        ...
        <b-btn
          data-test-id="capsule-button-startUpdate-Recovery"
          type="submit"
          variant="outline-dark"
          :disabled="isPageDisabled"
        >
        ...
        <b-btn
          data-test-id="capsule-button-startUpdate-Both"
          type="submit"
          variant="outline-dark"
          :disabled="isPageDisabled"
        >
...
export default {
  ...
  computed: {
    isTftpUploadAvailable() {
      return this.$store.getters['PFR/isTftpUploadAvailable'];
    },
  },
  watch: {
  ...
  created() {
    this.$store.dispatch('PFR/getUpdateServiceSettings');
  },
  methods: {
    updateFirmware() {
      this.startLoader();
    ...
    dispatchWorkstationUpload(timerId) {
      this.$store
        .dispatch('PFR/uploadFirmware', this.file)
        .catch(({ message }) => {
          this.endLoader();
    ...
    dispatchTftpUpload(timerId) {
      this.$store
        .dispatch('PFR/uploadFirmwareTFTP', this.tftpFileAddress)
        .catch(({ message }) => {
          this.endLoader();
          this.errorToast(message);
          clearTimeout(timerId);
